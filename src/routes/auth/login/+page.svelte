	...
	onMount(() => {
		...
		if (oauthError) {
			const errorMessages: Record<string, string> = {
				...
				oauth_only: 'This account uses Google sign-in. Please use the "Sign in with Google" button below.',
				google_access_denied: 'Google sign-in was cancelled.',
				server: 'Server error. Please try again later.'
			};
			error = errorMessages[oauthError] || 'An error occurred. Please try again.';
		}
	});
...
<div class="relative min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
	<div class="max-w-md w-full space-y-8">
		...
		<div class="glass-strong rounded-2xl p-8 border border-white/10 {mounted ? 'animate-fade-in-up' : 'opacity-0'}" style="animation-delay: 0.1s;">
			...
			<a
				href="/auth/google"
				class="mt-6 w-full group relative overflow-hidden rounded-xl p-3.5 glass border border-white/10 hover:border-white/20 transition-all duration-300 flex items-center justify-center gap-3"
			>
